...
from debussy_airflow.hooks.db_api import DbApiInterface, MySqlConnectorHook
from debussy_airflow.hooks.storage_hook import GCSHook
from debussy_airflow.operators.storage_to_rdbms import StorageToRdbmsOperator
from MySQLdb._exceptions import OperationalError

from test_tools import TestExceptionOperator, TestHookOperator, test_dag


def test_query_run_empty_list(context, hook_dbapi: MySqlConnectorHook, sql: str, autocommit: bool):

    assert hook_dbapi.query_run(sql=sql, autocommit=autocommit) == None or len(
        hook_dbapi.query_run(sql=sql, autocommit=autocommit)) == 0
...
def test_build_query_operator(context, operator: StorageToRdbmsOperator, dataset_table: pd.DataFrame):

    build_query = operator.build_insert_query(dataset_table=dataset_table)
    print(f"build_query: {build_query}")
    assert "INSERT INTO teste.participant(id_page,id_lead,name_campain,created_at_lead,is_client_dotz,celular) VALUES " in build_query


def get_id_page_incr(hook_dbapi: DbApiInterface):
    id_page_incr = hook_dbapi.query_run(sql="SELECT max(id_page)+1 FROM teste.participant", autocommit=True)
...
def get_pandas_dataset_participant_table(id_page_incr):
    dataset_table = pd.DataFrame({'id_page': [f'{id_page_incr}'],
                                  'id_lead': ['123456'],
...
with test_dag(
    ...
    test_storage_to_rdbms_operator = StorageToRdbmsOperator(dbapi_hook=dbapi_hook,
    ...
    test_build_query_storage_to_rdbms_operator = TestHookOperator(
        ...
        fn_kwargs={"operator": test_storage_to_rdbms_operator,
    ...
    test_sensor_exception = TestExceptionOperator(
        ...
        op_kwargs={'storage_hook': storage_hook,
