...
def test_upload_file(
    context, storage_hook: StorageHookInterface, remote_file_uri, local_file_uri
):
...
def test_upload_string(
    context, storage_hook: StorageHookInterface, remote_file_uri, data_string
):
...
def test_list_dir(context, storage_hook: StorageHookInterface, remote_uri):
    ...
    expected_file = sorted(["test_upload_file.txt", "test_upload_string.txt"])
...
with test_dag(dag_id="test_debussy_framework_storage") as dag:
    ...
    test_upload_string_gcs_hook = TestHookOperator(
        ...
        fn_kwargs={
            "storage_hook": gcs_storage_hook,
            "remote_file_uri": (
                "gs://autodelete_dev_bucket/test_debussy_framework_storage/test_upload_string.txt"
            ),
    ...
    test_upload_string_s3_hook = TestHookOperator(
        ...
        fn_kwargs={
            "storage_hook": s3_storage_hook,
            "remote_file_uri": (
                "s3://dotz-integracao-stg/test_debussy_framework_storage/test_upload_string.txt"
            ),
            "data_string": upload_string_text,
        },
        task_id="upload_string",
