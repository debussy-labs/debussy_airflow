...
link_paginated_hook = LinkPaginatedApiHook(
    method="GET", http_conn_id=conn_id, responses_handler_callable=responses_handler
)
...
with test_dag("test_debussy_api_dag") as dag:
    ...
    test_link_hook.fn_kwargs = {
        "log": test_link_hook.log,
        "hook": link_paginated_hook,
        "endpoint": "/response-headers",
        "data": {
            "link": '<https://postman-echo.com/response-headers?page=2>; rel="next"',
            "page": "1",
        },
    }
